...
class UniFormer(nn.Module):
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
                 ...
                 aux_head=False, num_aux_classes=46, sign_head=False, sign_groups=[], 
                 hier_head=False, num_hier_classes=40, hier_out=True, hier_sigmoid=False):
        """
        Args:
        ...
        self.sign_head = sign_head
        self.sign_groups = sign_groups
        self.hier_head = hier_head
        self.hier_out = hier_out
        self.hier_sigmoid = hier_sigmoid

        if hier_head:
            self.num_hier_classes = num_hier_classes
        if conv_stem:
        ...
        # Classifier head
        if self.hier_head:
            self.head_1 = nn.Linear(embed_dim[-1], self.num_hier_classes) if self.num_hier_classes > 0 else nn.Identity()
            self.head_2 = nn.Linear(self.num_hier_classes, self.num_classes) if self.num_classes > 0 else nn.Identity()
            self.sigmoid = nn.Sigmoid()
        elif self.sign_head:
            self.sign_cls_head = []
            for gidx in range(len(self.sign_groups)):
                sign_group = self.sign_groups[gidx]
                self.sign_cls_head.append(nn.Linear(embed_dim[-1], len(sign_group)))
            self.sign_cls_head = nn.ModuleList(self.sign_cls_head)
        else:
            self.head = nn.Linear(embed_dim[-1], num_classes) if num_classes > 0 else nn.Identity()
            if self.aux_head:
                self.auxiliary_head = nn.Linear(embed_dim[-1], self.num_aux_classes) if self.num_aux_classes > 0 else nn.Identity()
    ...
    def forward_MC_dropout(self, x):
        ...
        # print(self.hier_head)
        # print(self.hier_out)
        # print(self.aux_head)
        # print(self.sign_head)
        if self.hier_head:
            x = F.dropout(x,p=0.5)
            out2 = self.head_1(x)
...
def uniformer_base_mtask(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[5, 8, 20, 7],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), aux_head=True, **kwargs)
...
# sign group level multitask learning 
def uniformer_small_gmtask(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[3, 4, 8, 3],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
# hierarchical task ie. sign -> lesion
def uniformer_small_htask(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[3, 4, 8, 3],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), hier_head=True, **kwargs)
    model.default_cfg = _cfg()
    return model
...
# hierarchical task ie. sign -> lesion
def uniformer_base_htask(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[5, 8, 20, 7],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), hier_head=True, **kwargs)
...
def uniformer_small_IL_mtask(num_classes=2, 
    ...
    Concat multi-phase images with image-level
    '''
    model = uniformer_small_mtask(in_chans=num_phase, num_classes=num_classes,
                                num_aux_classes=num_aux_classes, **kwards)
    return model

@register_model
def uniformer_small_IL_gmtask(num_classes=2, 
                       sign_groups=[],
                       num_phase=10,
                       pretrained=None, 
                       pretrained_cfg=None,
    ...
    Concat multi-phase images with image-level
    '''
    model = uniformer_small_gmtask(in_chans=num_phase, num_classes=num_classes,
                                 sign_head=True, sign_groups=sign_groups, **kwards)
    return model

@register_model
def uniformer_small_IL_htask(num_classes=2, 
                       num_phase=10,
                       num_hier_classes=40,
                       hier_out=True,
                       hier_sigmoid=False,
    ...
    Concat multi-phase images with image-level
    '''
    model = uniformer_small_htask(in_chans=num_phase, num_classes=num_classes,
                                 num_hier_classes=num_hier_classes, hier_out=hier_out, hier_sigmoid=hier_sigmoid, **kwards)
    return model
...
@register_model
def uniformer_base_IL_mtask(num_classes=2, 
                       num_aux_classes=46,
                       num_phase=10,
                       pretrained=None, 
                       pretrained_cfg=None,
    ...
    Concat multi-phase images with image-level
    '''
    model = uniformer_base_mtask(in_chans=num_phase, num_classes=num_classes,
                                num_aux_classes=num_aux_classes, **kwards)
    return model

@register_model
def uniformer_base_IL_htask(num_classes=2, 
                       num_phase=10,
                       num_hier_classes=40,
                       hier_out=True,
                       hier_sigmoid=False,
    ...
    Concat multi-phase images with image-level
    '''
    model = uniformer_base_htask(in_chans=num_phase, num_classes=num_classes,
                                 num_hier_classes=num_hier_classes, hier_out=hier_out, hier_sigmoid=hier_sigmoid, **kwards)
    return model